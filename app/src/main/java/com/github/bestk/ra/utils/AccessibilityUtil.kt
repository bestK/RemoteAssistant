...
object AccessibilityUtil {
    ...
    fun scrollToTop(
        ...
        while (index++ < maxRetry) {
            ...
            if (scrollBefore == findAllOnceByClazz(getRoot(), Views.TextView)) {
                ...
                ToastUtils.showLong("未滚动到顶部$index")
                if (listener != null && listener.onScroll()) {
                    LogUtils.d("提前终止滚动")
                    ToastUtils.showLong("提前终止滚动")
                    return true
                }
            }
        }
        if (tryUseGesture) {
            ...
            ToastUtils.showLong("未找到可滚动列表 使用手势滚动")
    ...
    fun performClick(nodeInfo: AccessibilityNodeInfo?, retry: Boolean = true): Boolean {
        ...
        if (retry) {
            ...
            nodeInfo.refresh()
            val click = clickByNode(RemoteAssistantController.service, nodeInfo)
